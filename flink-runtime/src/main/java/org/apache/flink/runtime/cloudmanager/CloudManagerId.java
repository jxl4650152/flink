 ...
 */
public class CloudManagerId extends AbstractID {
	 ...
	 * Generates a new random ResourceManagerId.
	 */
	public static CloudManagerId generate() {
		return new CloudManagerId();
	}

	/**
	 * Creates a ResourceManagerId that corresponds to the given UUID.
	 */
	public static CloudManagerId fromUuid(UUID uuid) {
		return new CloudManagerId(uuid);
	}

	/**
	 * If the given uuid is null, this returns null, otherwise a ResourceManagerId that
	 * corresponds to the UUID, via {@link #CloudManagerId(UUID)}.
	 */
	public static CloudManagerId fromUuidOrNull(@Nullable UUID uuid) {
		return  uuid == null ? null : new CloudManagerId(uuid);
	}
}
