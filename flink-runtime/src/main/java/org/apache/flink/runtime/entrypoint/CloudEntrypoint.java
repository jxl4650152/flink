...
import org.apache.flink.runtime.cloudmanager.CloudManagerConfiguration;
 ...
 */
public abstract class CloudEntrypoint implements AutoCloseableAsync, FatalErrorHandler {
	...
	protected static final Logger LOG = LoggerFactory.getLogger(CloudEntrypoint.class);
	...
	protected CloudEntrypoint(Configuration configuration) {
...
//			configureFileSystems(configuration);
			...
			throw new ClusterEntrypointException(
				String.format("Failed to initialize the cloud entrypoint %s.", getClass().getSimpleName()),
				strippedThrowable);
		}
	}
	...
	private void runCluster(Configuration configuration) throws Exception {
		synchronized (lock) {
			...
			CloudManagerConfiguration cmc = CloudManagerConfiguration.fromConfiguration(configuration);
			ResourceID rId = ResourceID.generate();
			CloudManager cm = new CloudManager(commonRpcService,rId, this::onFatalError,haServices,cmc);
			cm.start();
		}
	}
	...
	protected void initializeServices(Configuration configuration) throws Exception {
		...
		LOG.info("Initializing cloud services.");

		synchronized (lock) {
			final String bindAddress = configuration.getString(CloudManagerOptions.ADDRESS);
			final String portRange = getRPCPortRange(configuration);

			commonRpcService = createRpcService(configuration, bindAddress, portRange);

			// update the configuration used to create the high availability services
			configuration.setString(CloudManagerOptions.ADDRESS, commonRpcService.getAddress());
			configuration.setInteger(CloudManagerOptions.PORT, commonRpcService.getPort());
...
//			archivedExecutionGraphStore = createSerializableExecutionGraphStore(configuration, commonRpcService.getScheduledExecutor());
	...
	protected String getRPCPortRange(Configuration configuration) {
		if (ZooKeeperUtils.isZooKeeperRecoveryMode(configuration)) {
			...
			return String.valueOf(configuration.getInteger(CloudManagerOptions.PORT));
		}
	}
	...
	/**
	 * Clean up of temporary directories created by the {@link CloudEntrypoint}.
	 *
	...
	public static void runCloudEntrypoint(CloudEntrypoint cloudEntrypoint) {

		final String cloudEntrypointName = cloudEntrypoint.getClass().getSimpleName();
		try {
			cloudEntrypoint.startCluster();
		} catch (ClusterEntrypointException e) {
			LOG.error(String.format("Could not start cluster entrypoint %s.", cloudEntrypointName), e);
			System.exit(STARTUP_FAILURE_RETURN_CODE);
		}

		cloudEntrypoint.getTerminationFuture().whenComplete((applicationStatus, throwable) -> {
			...
			LOG.info("Terminating cluster entrypoint process {} with exit code {}.", cloudEntrypointName, returnCode, throwable);
